...
func EmulationUpdate(CacheCfg *cache.CacheInfo) error {
	...
	if err != nil {
		logger.Errorf("findmnt failed : %v", err)
	}

	logger.Debugf("%+v", outAppend)
	...
	// logger.Debugf("%+v", sourcePoint)

	// create mount point
	mountPointRootPath, err := ioutil.TempDir("/tmp/", "rootfs-point_")
	if err != nil {
		logger.Errorf("create rootfs point failed %v", err)
	}
	logger.Debugf("%+v", mountPointRootPath)
	...
	func() {
		func() {
			if err := fs.CreateDirMode(mountPointRootPath+"/upperdir", 0755); err != nil {
				logger.Errorf("error creating mount point upperdir: %v", err)
			}
	...
	if err != nil {
		logger.Errorf("Error creating temporary %+v", err)
	}
	...
	emulation := EmulationMainShell{
		...
		SourcePoint: func() []string {
			...
			logger.Debugf("Filtered: %v", filtered)
			return filtered
		}(),
	}

	emulation.Verbose = true
	logger.Debugf("Emulation started %v", emulation)
