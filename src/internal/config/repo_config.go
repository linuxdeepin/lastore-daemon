...
var (
	isFileExistFn = utils2.IsFileExist
	moveFileFn    = utils2.MoveFile
	removeAllFn   = os.RemoveAll

	getOemRepoInfoFn = GetOemRepoInfo

	updateSystemDefaultSourceDirFn   = system.UpdateSystemDefaultSourceDir
	updateSecurityDefaultSourceDirFn = system.UpdateSecurityDefaultSourceDir
	updateSourceDirUseUrlFn          = system.UpdateSourceDirUseUrl
)
...
func (c *Config) recoveryAndApplyOemFlag(typ system.UpdateType) error {
	...
	return removeAllFn(flagFilePath)
}

func (c *Config) reloadOemRepoConfig() {
	systemOemSourceConfig, securityOemSourceConfig := getOemRepoInfoFn(OemRepoDirPath)
...
func (c *Config) ReloadSourcesDir() {
	...
	case OSDefaultRepo:
		err = updateSystemDefaultSourceDirFn(c.SystemSourceList)
	case OemDefaultRepo:
		err = updateSourceDirUseUrlFn(system.SystemUpdate, c.SystemOemSourceConfig.RepoUrl, "system-oem-sources.list", "Oem system repo config, Generated by lastore-daemon")
	case CustomRepo:
		err = updateSourceDirUseUrlFn(system.SystemUpdate, c.SystemCustomSource, "system-custom-sources.list", "Custom system repo config, Generated by lastore-daemon")
	}
	if err != nil {
	...
	case OSDefaultRepo:
		err = updateSecurityDefaultSourceDirFn(c.SecuritySourceList)
	case OemDefaultRepo:
		err = updateSourceDirUseUrlFn(system.SecurityUpdate, c.SecurityOemSourceConfig.RepoUrl, "security-oem-sources.list", "Oem security repo config, Generated by lastore-daemon")
	case CustomRepo:
		err = updateSourceDirUseUrlFn(system.SecurityUpdate, c.SecurityCustomSource, "security-custom-sources.list", "Custom security repo config, Generated by lastore-daemon")
	}
