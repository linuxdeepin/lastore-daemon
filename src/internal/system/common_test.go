...
func Test_getGrubTitleByPrefix(t *testing.T) {
	title := getGrubTitleByPrefix("./testdata/grub.cfg", "BEGIN /etc/grub.d/10_linux")
	assert.Equal(t, "UnionTech OS Desktop 20 Pro GNU/Linux", title)
	title = getGrubTitleByPrefix("./testdata/grub.cfg", "BEGIN /etc/grub.d/11_deepin_ab_recovery")
