...
func (m *Manager) retryDistUpgrade(upgradeJob *Job, needBackup bool) error {
	...
	if backupJob != nil {
		if !backupJob.IsStatus(system.FailedStatus) {
			return errors.New("backup job status is not failed")
		}
		if needBackup {
			// retry backup job
			err := m.jobManager.MarkStart(backupJob.Id)
			if err != nil {
				logger.Warningf("retryDistUpgrade: mark start backup job failed: %v", err)
				return err
			}
		} else {
			// delete backup job
			err := m.jobManager.CleanJob(backupJob.Id)
			if err != nil {
				logger.Warningf("retryDistUpgrade: clean backup job failed: %v", err)
				return err
			}

			// retry upgrade job
			err = m.jobManager.MarkStart(upgradeJob.Id)
			if err != nil {
				logger.Warningf("retryDistUpgrade: mark start upgrade job failed: %v", err)
				return err
			}
		}
	} else {
		logger.Debug("backup job is not found")
		err := m.jobManager.MarkStart(upgradeJob.Id)
		if err != nil {
			logger.Warningf("retryDistUpgrade: mark start upgrade job failed: %v", err)
			return err
